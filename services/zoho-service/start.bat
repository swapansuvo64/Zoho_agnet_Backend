@echo off
cd /d "%~dp0"
echo Starting Zoho Agent Zoho Service on Windows...
...
for /f "tokens=5" %%a in ('netstat -aon ^| findstr :8003 ^| findstr LISTENING') do (
    echo Found process %%a on port 8003. Killing it to start fresh...
    taskkill /f /pid %%a
)

echo Starting FastAPI server on port 8003...
venv\Scripts\python.exe -m uvicorn src.main:app --host 0.0.0.0 --port 8003 --reload
pause
